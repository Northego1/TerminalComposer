...
target. Для shell-адаптера это `ESC`: TUI-агенты по нему останавливают текущий
шаг, а в zsh одиночный `ESC` безобиден (отбрасывается по `KEYTIMEOUT`).
`Ctrl+C` намеренно не шлём — случайный `Esc` не должен убивать процесс.

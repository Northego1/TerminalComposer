...
export function useCompletion(editor: Editor | null, commandLine: boolean) {
  ...
  const lookUp = useCallback(
    (forced: boolean) => {
      ...
      const asPath = !current.first || LOOKS_LIKE_PATH.test(current.text);
      const welcome = forced || (asPath ? LOOKS_LIKE_PATH.test(current.text) : commandLine);
      if (!welcome) return close();

      token.current = current;
      const request = asPath
        ? invoke<PathCompletion[]>("complete_path", {
            id: sessionId,
            token: current.text,
          }).then((items) =>
            items.map((item) => ({
              ...
              kind: item.isDir ? ("dir" as const) : ("file" as const),
            })),
          )
        : invoke<string[]>("complete_command", {
            id: sessionId,
            prefix: current.text,
          }).then((names) =>
            names.map((name) => ({
